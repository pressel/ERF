...
void SAM::IceFall (const SolverChoice& sc) {
    ...
    for (MFIter mfi(fz, TileNoZ()); mfi.isValid(); ++mfi) {
        ...
        ParallelFor(box3d, [=] AMREX_GPU_DEVICE(int i, int j, int k) noexcept
        {
            ...
            const SAMFaceState face_state = sam_face_average_state_current_behavior(k, k_lo, k_hi,
                                                                                    rho_km1, rho_k,
                                                                                    zero, zero,
                                                                                    qci_km1, qci_k,
                                                                                    zero, zero);
            const Real vt_ice = sam_cloud_ice_terminal_velocity_current_behavior(face_state.qci_avg);
    ...
    for (int nsub(0); nsub<n_substep; ++nsub) {
        for (MFIter mfi(*qci, TileNoZ()); mfi.isValid(); ++mfi) {
            ...
            ParallelFor(tbx, [=] AMREX_GPU_DEVICE(int i, int j, int k) noexcept
            {
                ...
                //==================================================
                Real dqi  = sam_sedimentation_tendency_current_behavior(fz_array(i,j,k+1), fz_array(i,j,k),
