...
foreach(_nranks IN ITEMS 1 2)
    ...
    list(APPEND _command erf.sbm_composite_diagnostic_file=${_diagnostic})
    execute_process(
        COMMAND ${_command}
    ...
    endforeach()
    string(REGEX MATCH "post_reflux_validation_count=([1-9][0-9]*)" _validation_match "${_diagnostic_text}")
    if(NOT _validation_match)
        message(FATAL_ERROR "${_nranks}-rank subcycling diagnostic did not record a positive post-reflux validation count")
    endif()
    string(REGEX MATCH "post_reflux_material_rejection_count=0([^0-9]|$)" _material_match "${_diagnostic_text}")
    if(NOT _material_match)
        message(FATAL_ERROR "${_nranks}-rank subcycling diagnostic recorded a material post-reflux rejection")
    endif()
